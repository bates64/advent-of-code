...
pub fn part2() -> Result<String, Error> {
    ...
    for line in INPUT.lines() {
        ...
        for c in line.chars() {
            ...
            state.push(c);
            if n.is_none() {
                n = if state.ends_with("one") { Some(1) }
                    else if state.ends_with("two") { Some(2) }
                    else if state.ends_with("three") { Some(3) }
                    else if state.ends_with("four") { Some(4) }
                    else if state.ends_with("five") { Some(5) }
                    else if state.ends_with("six") { Some(6) }
                    else if state.ends_with("seven") { Some(7) }
                    else if state.ends_with("eight") { Some(8) }
                    else if state.ends_with("nine") { Some(9) }
                    else if state.ends_with("zero") { Some(0) }
                    else { None };
            }
